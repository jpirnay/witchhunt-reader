// Ported from crosspoint-reader PR #3203 "feat: add usb mass storage mode for
// x4 pro" (Julia <julia@uxj.io>, with Justin Mitchell and Uri Tauber).
// Kept register-for-register identical to upstream: this reproduces Arduino's
// own usb_switch_to_cdc_jtag() sequence, and every line of it is load-bearing.

#include "UsbSerialJtagHandoff.h"

// BoardConfig defines FREEINK_CAP_USB_MSC to 0 when a board has not opted in.
// It must be included BEFORE the guard below: without it the capability would
// read as undefined on any build that sets it through BoardConfig rather than a
// -D flag, and this file would quietly compile to the no-op stub — leaving the
// host enumerating a dead USB device after every USB Drive session.
