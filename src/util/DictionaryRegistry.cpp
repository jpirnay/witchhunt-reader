...
//
// The prefix exists only so the host test can point both roots at a temp
// directory -- the SD card has no writable equivalent of "/dictionaries" on a
// developer machine. Firmware builds never define it, so it compiles away to
// nothing and costs no RAM or runtime.
#ifndef DICTIONARY_ROOT_PREFIX
#define DICTIONARY_ROOT_PREFIX ""
#endif
constexpr const char* DICT_ROOTS[] = {DICTIONARY_ROOT_PREFIX "/dictionaries", DICTIONARY_ROOT_PREFIX "/.dictionaries"};
