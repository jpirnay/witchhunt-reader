...
void GfxRenderer::drawBitmap(const Bitmap& bitmap, const int x, const int y, const int maxWidth, const int maxHeight,
  ...
  // Resolve draw mask from render mode (darkness not applicable to images — treat as darkness=1)
  const uint8_t drawMask = drawMaskFor2BitMode(renderModeSnapshot, 1);
  ...
  for (int bmpY = 0; bmpY < (bitmap.getHeight() - cropPixY); bmpY++) {
    ...
    if (!isScaled && drawMask != 0x00) {
      ...
      switch (drawMask) {
        case 0x0E:
          bitmapFastRow<0x0E>(frameBuffer, outputRow, cropPixX, renderWidth, x, screenY, orientation, pixelState,
...
void GfxRenderer::drawBitmap1Bit(const Bitmap& bitmap, const int x, const int y, const int maxWidth,
  ...
  for (int bmpY = 0; bmpY < bitmap.getHeight(); bmpY++) {
    ...
    if (!isScaled) {
      ...
      bitmapFastRow<0x0E>(frameBuffer, outputRow, 0, bitmap.getWidth(), x, screenY, orientation, true, displayWidth,
                          displayHeight, widthBytes);
      continue;
    }
